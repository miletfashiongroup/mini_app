# Импорт каталога (VPS) из products_brace.xlsx

Ниже команды для замены всех данных каталога на новые из файла
`/root/brace__1/products_brace.xlsx`.

## 1) Залить файл на VPS (с macOS)

```bash
scp -i ~/.ssh/braceTG /Users/shmnn/brace__1/brace__1/products_brace.xlsx root@79.174.93.119:/root/brace__1/products_brace.xlsx
```

## 2) Применить миграции (на VPS)

```bash
docker exec infra-backend-1 alembic upgrade head
```

## 3) Импорт данных и очистка аналитики (на VPS)
...
def parse_array(val):
    ...
    if s == "":
        return "ARRAY[]::varchar[]"
    parts = [p.strip().lstrip('#%') for p in s.split(';') if p.strip().lstrip('#%')]
    if not parts:
        return "ARRAY[]::varchar[]"
...
for p in products:
    sql.append(
        "INSERT INTO products (id, product_code, name, description, hero_media_url, category, is_new, tags, specs, is_deleted) VALUES "
        f"({sql_str(prod_id[p['product_code']])},"
        f"{sql_nullable(p.get('product_code',''))},"
        f"{sql_str(p['name'])},"
...
psql -h 79.174.88.146 -p 18279 -U brace_user -d brace_tg -f /tmp/brace_import.sql
```
