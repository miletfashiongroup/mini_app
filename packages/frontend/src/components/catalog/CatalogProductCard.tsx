...
const CatalogProductCard = ({ isNew, tags, price, ratingCount, ratingValue }: CatalogProductCardProps) => {
  return (
    <div className="flex flex-col">
      ...
      <div className="mt-2 flex flex-nowrap gap-x-1.5 gap-y-1.5">
        {tags.map((tag) => (
          <span
            key={tag}
            className="whitespace-nowrap rounded-full bg-[#F3F3F7] px-2 py-1 text-[10px] font-medium leading-tight text-[#29292B]"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="mt-2 flex items-center justify-between gap-2 min-w-0">
        ...
        <div className="flex items-center gap-2 text-[12px] leading-none whitespace-nowrap">
          <span className="font-medium text-[#BABABA]">{ratingCount} оценки</span>
