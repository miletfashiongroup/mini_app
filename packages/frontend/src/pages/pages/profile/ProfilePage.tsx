...
export const ProfilePage = () => {
  ...
  const sections = useMemo<ProfileSectionItemProps[]>(
    () => [
      ...
      { label: 'бонусная система', icon: giftIcon, onClick: () => navigate('/profile/bonus') },
      { label: 'реферальная система', icon: handsIcon, onClick: () => navigate('/profile/referrals') },
      { label: 'поддержка', icon: supportIcon, onClick: () => navigate('/support') },
      { label: 'юридические документы', icon: docsIcon, onClick: () => navigate('/profile/legal') },
    ],
    [navigate],
  );
