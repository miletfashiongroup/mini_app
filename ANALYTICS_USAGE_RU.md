...
- `BRACE_ANALYTICS_DASHBOARD_DAILY_URL=...` (опционально)
- `BRACE_ANALYTICS_DASHBOARD_WEEKLY_URL=...` (опционально)
- `BRACE_ANALYTICS_DASHBOARD_MONTHLY_URL=...` (опционально)
...
poetry run python -m brace_backend.scripts.analytics_report --type monthly
...
10 5 1 * * /usr/bin/env BRACE_DATABASE_URL=... BRACE_ANALYTICS_REPORT_ENABLED=true BRACE_ANALYTICS_REPORT_RECIPIENT_IDS=123 poetry run python -m brace_backend.scripts.analytics_report --type monthly
poetry run python -m brace_backend.scripts.analytics_report --type daily --dry-run
```
