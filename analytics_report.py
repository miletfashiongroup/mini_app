...
def _monthly_message(session: Session, target_date: date) -> str:
    ...
    return "\n".join(
        [
            f"Ежемесячный отчет: {start_date.isoformat()} — {target_date.isoformat()}",
            f"Сессии: {int(sessions)}",
            f"Просмотры товаров: {int(product_views)}",
            f"Добавления в корзину: {int(add_to_cart)}",
            f"Начат чек‑аут: {int(checkout_start)}",
...
def _dashboard_url(report_type: str) -> str | None:
    if report_type == "daily":
        return settings.analytics_report_dashboard_daily_url
    if report_type == "weekly":
        return settings.analytics_report_dashboard_weekly_url
    if report_type == "monthly":
        return settings.analytics_report_dashboard_monthly_url
    return None


def _append_dashboard_link(message: str, report_type: str) -> str:
    url = _dashboard_url(report_type)
    if not url:
        return message
    return "\n".join([message, "", f"Дашборд: {url}"])
...
def run_report(report_type: str, target_date: date, dry_run: bool) -> None:
    ...
    with Session(engine) as session:
        if report_type == "daily":
            ...
            message = _monthly_message(session, target_date)
        message = _append_dashboard_link(message, report_type)
